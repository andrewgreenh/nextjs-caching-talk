...
async function MyTeamGrid() {
  ...
  return (
    <div {...compId("MyTeamGrid")} className={myTeamGridClass}>
      {details.map((p) => (
        <article key={p.id} className={myTeamCardClass}>
          <div className={myTeamCardImageWrapperClass}>
            <img src={p.sprite} alt={p.name} className={myTeamCardImageClass} loading="lazy" />
          </div>
          <h2 className={myTeamCardTitleClass}>
            {p.name}
            <span className={myTeamCardIdBadgeClass}>#{p.id}</span>
          </h2>
          <form action={removeFromMyTeam} className={myTeamRemoveFormClass}>
            <input type="hidden" name="name" value={p.name} />
            <button type="submit" className={myTeamRemoveButtonClass}>
              Remove
            </button>
          </form>
        </article>
...
const myTeamMainClass = "mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10";
...
const myTeamGridClass = "grid gap-6 sm:grid-cols-2 md:grid-cols-3";
...
const myTeamCardImageWrapperClass = "flex items-center justify-center mb-4 h-32";
const myTeamCardImageClass =
  "h-24 w-24 object-contain drop-shadow-md transition group-hover:scale-105";
const myTeamCardTitleClass = "text-lg font-semibold capitalize flex items-center gap-2 mb-3";
