  ...
  return (
    <div className={allPokemonWrapperClass}>
          ...
          {initialList.map((p) => (
            <li key={p.name}>
              <button
                ...
                className={
                  selected === p.name ? allPokemonListItemActiveClass : allPokemonListItemClass
                }
              >
                {p.name}
              </button>
      ...
      <section className={allPokemonDetailSectionClass}>
        {!selected && (
          <p className={allPokemonIndexParagraphClass}>Select a Pokémon to see details.</p>
        )}
        {selected && loading && (
          <p className={allPokemonLoadingParagraphClass}>Loading {selected}...</p>
        )}
        {error && <p className={allPokemonErrorParagraphClass}>Error: {error}</p>}
        {detail && !loading && !error && (
          <div className={allPokemonCardWrapperClass}>
            <div className={allPokemonCardImageWrapperClass}>
              <img
                src={detail.sprite}
                alt={detail.name}
            ...
            <h2 className={allPokemonCardTitleClass}>
              {detail.name}
              <span className={allPokemonCardIdBadgeClass}>#{detail.id}</span>
            </h2>
            <button
              type="button"
...
const allPokemonCardImageWrapperClass = "flex items-center justify-center mb-4 h-32";
const allPokemonCardImageClass = "h-28 w-28 object-contain drop-shadow-md";
const allPokemonCardTitleClass = "text-xl font-bold capitalize mb-2 flex items-center gap-2";
