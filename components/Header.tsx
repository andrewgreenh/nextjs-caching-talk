...
export function Header() {
  return (
    <header {...compId("Header")} className={headerWrapperClass}>
      <div className={headerInnerClass}>
        <div className={headerBrandWrapperClass}>
          <Link href="/" className={headerBrandLinkClass}>
            <span className={headerBrandGradientSpanClass}>Next.js Caching</span>
          </Link>
        </div>
        <nav aria-label="Main navigation" className={headerNavClass}>
            ...
            {links.map(({ href, label, exact }) => (
              <li key={href}>
                <NavLink
                  ...
                  inactiveClassName={headerNavLinkInactiveClass}
...
const headerInnerClass = "mx-auto flex h-16 max-w-7xl items-center px-4 sm:px-6 lg:px-8";
...
const headerNavListClass = "flex items-center gap-2 sm:gap-4";
