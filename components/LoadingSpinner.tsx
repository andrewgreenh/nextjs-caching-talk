...
export function LoadingSpinner({
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className={[loadingSpinnerWrapperClass, className]
        .filter(Boolean)
        .join(" ")}
    >
      <span
        className={loadingSpinnerIndicatorClass}
        style={{ width: size, height: size }}
      />
      <span className={loadingSpinnerSrOnlyClass}>
        {label}
      </span>
    </div>
  );
...
const loadingSpinnerWrapperClass =
